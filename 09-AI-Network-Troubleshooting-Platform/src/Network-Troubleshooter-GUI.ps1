...
$Engine   = Join-Path $Desktop "AI-Offline.ps1"
...
$form.Text = "Network Troubleshooter v5.2 CLEAN"
...
$version.Text = "v5.2  |  Local / Offline"
$version.Dock = "Fill"
...
$btnHybrid.Add_Click({
    $txtResult.Text = "Hybrid AI analysis running..."
    $form.Refresh()

    $files = @(Get-SelectedEvidenceFiles)

    $txtResult.Text = Invoke-TroubleshooterEngine `
...
$historyLayout.RowStyles.Add((New-Object System.Windows.Forms.RowStyle("Percent",100)))
...
$historyLayout.Controls.Add($historyActions,0,2)
...
function Refresh-SystemStatus {
    ...
    $lines += "NETWORK TROUBLESHOOTER v5.2"
    $lines += ""
