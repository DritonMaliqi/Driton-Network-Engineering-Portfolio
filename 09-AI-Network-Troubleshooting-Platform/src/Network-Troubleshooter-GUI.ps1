...
$form.Text = "NETOPS Network Troubleshooter v5.8"
...
$version.Text = "v5.8 | Dashboard Integration / Smart History"
$version.Dock = "Fill"
...
$btnHybrid.Add_Click({
    ...
    # NETOPS v5.4 HYBRID AUTO-SAVE
    if (
        (Get-Command Save-NetOpsIncident -ErrorAction SilentlyContinue) -and
        $txtResult.Text -match '(?im)^FINDINGS:\s*[1-9]'
    ) {
        try {
            $script:CurrentIncident = Save-NetOpsIncident `
                -Description $txtPrompt.Text `
                -Analysis $txtResult.Text `
                -Status "Open"
